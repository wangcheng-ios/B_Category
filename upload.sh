...
Repo="wangspec"
RepoList=("wangspec" "wangspec_test" "wangspec_gray")
getRepo() {
    echo -e "\n"
    length=${#RepoList[@]}
...
TestVersionLineNumber=`grep -nE 'version.test.*=' B_Category.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" B_Category.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' B_Category.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" B_Category.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' B_Category.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" B_Category.podspec
...
pod repo push ${Repo} B_Category.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
